...
if __name__ == '__main__':
    ...
    env = sumoEnv({'algo':f'{options.algorithm}-repair-{options.repair}days_maintain-{options.maintain}hours_density-{options.density}','repair':options.repair,'maintain':options.maintain,'map':options.density})
    # init the env
    obs = env.reset()
    reward = 0
    done_state = False
